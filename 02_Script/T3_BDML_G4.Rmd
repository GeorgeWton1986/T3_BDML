...
# Reinicia tu sesión de R (Ctrl + Shift + F10 en RStudio)

# Asegurar que parsnip esté cargado
library(tidymodels)

# Dividir en train y validación (80%-20%)
set.seed(123)
split <- initial_split(train_factors, prop = 0.8)
train_data <- training(split)
valid_data <- testing(split)

# Crear receta (sin transformar variables, pero puedes agregar pasos si deseas)
receta <- recipe(price ~ ., data = train_data)

# Definir el modelo CART
modelo_cart <- decision_tree(mode = "regression") %>%
  set_engine("rpart")
...
# Entrenar el modelo
fit_cart <- fit(workflow_cart, data = train_data)

# Predecir sobre validación
predicciones_val <- predict(fit_cart, new_data = valid_data) %>%
  bind_cols(valid_data %>% select(price))

# Evaluar desempeño
metrics(predicciones_val, truth = price, estimate = .pred)
...
test$pred_CART_cv <- predict(cart_fit, new_data = test) %>% pull(.pred)

# 2.2 (Opcional) Evaluación si tienes `price` real en test
# mae(test, truth = price, estimate = pred_CART_cv)
