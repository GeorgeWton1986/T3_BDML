...
receta_rf <- recipe(price ~ ., data = train_espacial) %>%
  update_role(property_id, new_role = "ID") %>%    # property_id solo como ID
  step_rm(geometry) %>%                            # eliminar columna geometry antes del modelo
  step_dummy(all_nominal_predictors()) %>%        # dummies para categóricas
  step_zv(all_predictors())                        # quitar variables sin variación

# 3. Definir el modelo Random Forest con parámetros para tunear
modelo_rf <- rand_forest(
  mtry = tune(),        # número de variables a probar en cada split
  trees = 1000,         # más árboles para estabilidad
  min_n = tune()        # tamaño mínimo de nodo
) %>%
  set_engine("ranger", importance = "permutation") %>%  # importancia para análisis posterior
  set_mode("regression")

# 4. Armar workflow combinando receta y modelo
workflow_rf <- workflow() %>%
  add_recipe(receta_rf) %>%
  add_model(modelo_rf)

# 5. Crear grid de hiperparámetros para búsqueda
grid_rf <- grid_regular(
  mtry(range = c(2, floor(sqrt(ncol(train_espacial) - 2)))),  # rango dinámico mtry
  min_n(range = c(5, 20)),                                    # valores más realistas para min_n
  levels = 5
)

# 6. Ejecutar tuning con validación cruzada espacial
resultados_rf <- tune_grid(
  workflow_rf,
  resamples = block_folds_rf,
  grid = grid_rf,
  metrics = metric_set(rmse, rsq, mae),   # métricas regresión
  control = control_grid(save_pred = TRUE, verbose = TRUE)
)

# 7. Revisar resultados y seleccionar mejores hiperparámetros (menor RMSE)
mejores_params <- select_best(resultados_rf, metric = "rmse")

# 8. Finalizar workflow con mejores parámetros y entrenar modelo final
final_workflow_rf <- finalize_workflow(workflow_rf, mejores_params)

final_model_rf <- fit(final_workflow_rf, data = train_espacial)

# 9. Importancia de variables (opcional)
importancia <- final_model_rf %>%
  extract_fit_parsnip() %>%
  vip::vip(geom = "col")
```
