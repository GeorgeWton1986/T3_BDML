...
test_rn$zona <- zonas$cluster[(nrow(train_factors)+1):nrow(data_geo)]
...
pred_rn1 <- predict(final_fit, new_data = test_rn)

resultado_rn1 <- test_rn %>%
  select(property_id) %>%
  bind_cols(data.frame(price = pred_rn1))

# Ver la distribución de las predicciones
ggplot(resultado_rn1, aes(x = .pred/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
...
# Preprocesar test
test_rn <- test %>%
  left_join(test_aptos %>% select(property_id, lat, lon), by = "property_id")

x_test <- bake(receta_prep, new_data = test_rn) %>%
  as.matrix()

# Generar predicciones
pred_rn2 <- modelo_rn2 %>% predict(x_test)

resultado_rn2 <- test_rn %>%
  select(property_id) %>%
  bind_cols(data.frame(price = pred_rn2))

# Ver la distribución de las predicciones
ggplot(resultado_rn2, aes(x = price/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
...
resultado_rn3 <- test_rn %>%
  select(property_id) %>%
  bind_cols(data.frame(price = pred_rn3))

# Ver la distribución de las predicciones
ggplot(resultado_rn3, aes(x = price/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
