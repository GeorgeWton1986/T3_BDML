...
##Localidades sin Chapiner (NICOLAS)

Identificar distancia a parques

```{r}
# Obtener los límites generales de Bogotá
limites_bogota <- getbb("Bogotá, Colombia")

# Extraer todos los parques en Bogotá
parques_bogota <- opq(bbox = limites_bogota) %>%
  add_osm_feature(key = "leisure", value = "park")

# Convertir a formato sf
parques_sf_bogota <- osmdata_sf(parques_bogota)

# Seleccionar la geometría relevante (puede ser polygons o multipolygons según el resultado)
parques_bogota_geometria <- parques_sf_bogota$osm_polygons %>%
  dplyr::select(osm_id, name)

leaflet() %>%
  addTiles() %>%
  addPolygons(data = parques_bogota_geometria, color = "red", weight = 2, opacity = 0.8, popup = parques_bogota_geometria$name)

```


```{r}
# Calcular los centroides de los parques en Bogotá
centroides_bogota <- st_centroid(parques_bogota_geometria, byid = TRUE)
...
# Descargar estaciones de TransMilenio en Bogotá
...
```{r}
# Obtener bbox de Bogotá
limites_bogota <- getbb("Bogotá, Colombia")

# Obtener CAI (policías) en Bogotá
cai_query_bogota <- opq(bbox = limites_bogota) %>%
  add_osm_feature(key = "amenity", value = "police")

# Obtener datos de CAI
cai_data_bogota <- osmdata_sf(cai_query_bogota)
cai_sf_bogota <- cai_data_bogota$osm_points %>% dplyr::select(osm_id, name)
cai_sf_bogota <- st_transform(cai_sf_bogota, 4326)

# Convertir apartamentos fuera de Chapinero a sf
aptos_sf_sin_chapinero <- st_as_sf(train_localidades, coords = c("lon", "lat"), crs = 4326)

# Calcular distancias entre los apartamentos y los CAI de Bogotá
dist_matrix_cai_sin_chapinero <- st_distance(aptos_sf_sin_chapinero, cai_sf_bogota)
dist_min_cai_sin_chapinero <- apply(dist_matrix_cai_sin_chapinero, 1, min)

# Agregar la distancia mínima a la base de datos
train_localidades <- train_localidades %>%
  mutate(distancia_cai = dist_min_cai_sin_chapinero)
...
```{r}
# Definir el área de Bogotá (cubre todas las localidades)
limites_bogota <- getbb("Bogotá, Colombia")

# Descargar vías principales en Bogotá
vias_query_bogota <- opq(bbox = limites_bogota) %>%
  add_osm_feature(key = "highway", value = c("primary", "trunk"))

# Obtener datos como sf
vias_data_bogota <- osmdata_sf(vias_query_bogota)

# Usar líneas de vías principales si están disponibles
vias_sf_bogota <- vias_data_bogota$osm_lines %>% dplyr::select(osm_id, name)
vias_sf_bogota <- st_transform(vias_sf_bogota, 4326)

# Convertir apartamentos a sf si no lo están
aptos_sf_bogota <- st_as_sf(train_localidades, coords = c("lon", "lat"), crs = 4326)

# Calcular distancias a las avenidas principales más cercanas
dist_matrix_vias_bogota <- st_distance(aptos_sf_bogota, vias_sf_bogota)
dist_min_vias_bogota <- apply(dist_matrix_vias_bogota, 1, min)

# Agregar a la base de datos de apartamentos en Bogotá
train_localidades <- train_localidades %>%
  mutate(distancia_via_principal = dist_min_vias_bogota)

```


##CHAPINERO (ZAIRA Y LAURA)
