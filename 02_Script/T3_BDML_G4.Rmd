...
Seleccionar las viviendas de la localidad de Chipenaro para Test 
```{r}
...
dummies_df_test <- test_chapinero_tokens_clean %>%
  mutate(binaria_parking_test = word %in% parking,
         binaria_terrace_test = word %in% terrace,
         binaria_gym_test = word %in% gym,
         binaria_laundry_test = word %in% laundry,
         binaria_elevator_test = word %in% elevator) %>%
  group_by(property_id) %>%
  summarise(across(starts_with("binaria_"), ~as.integer(any(.))))
...
train_localidades_EN <- st_drop_geometry(train_localidades_sf) %>%
  select(-c(property_id, title, description))

#2. Crear la receta del entranamiento
# Cargar explícitamente el paquete recipes
library(recipes)
receta_1_EN <- recipe (price ~., data = train_localidades_EN) %>%
  step_normalize(all_predictors()) %>% # Convertir variables categóricas a dummies
  step_dummy(all_nominal_predictors()%>% # Escalar predictores numéricos
...
#Penalty: lambda o alpha que controla la regularización
#Mixture: es el balance entre la regularizacion L1 (Lasso) y L2 (Ridge)
...
#6. Estimacion de validacion cruzada espacial por bloques
...
set.seed(123)
block_folds_EN <- spatial_block_cv(train_localidades_sf, v = 5)

#7. Verificación visual de los bloques de la valiación cruzada
autoplot(block_folds_EN)

#8. Incluir la libreria purrr
#Vamos a ver el proceso de la validacion cruzada 
library(purrr)
walk(block_folds$splits, function(x) print(autoplot(x)))

#9. Entrenamiento del modelo con validacion cruzada espacial
...
#10. Ver los resultados del modelo 1
