...
Distancia mínima de cada apto a una avenida ppal
...
## 1.1 Inspección y tratamiento de BD test
...
```{r}
limites_test <- getbb("Chapinero, Bogotá, Colombia")
limites_test

test_chapinero <- test_aptos %>%
  filter(
    between(lon, limites_test[1, "min"], limites_test[1, "max"]) & 
    between(lat, limites_test[2, "min"], limites_test[2, "max"])
  )
...
1)  Distancia a parques- Formato espacial Base chapinero

```{r}
# Asegúrate de que test_aptos tenga coordenadas en formato espacial (sf)
test_chapinero_sf <- st_as_sf(test_chapinero, coords = c("lon", "lat"), crs = 4326)

# Crear un buffer de 500 metros alrededor de cada apartamento (esto reduce el área de consulta)
test_buffer_aptos <- st_buffer(test_chapinero_sf, dist = 500)
