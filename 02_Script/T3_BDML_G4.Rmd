...
# Receta para preparar los datos (sin usar property_id ni geometry)
receta_cart <- recipe(price ~ ., data = train_espacial) %>%
  update_role(property_id, new_role = "id") %>%  # property_id no es predictor
  step_rm(geometry) %>%                          # geometry no es predictor
  step_dummy(all_nominal_predictors()) %>%      # convertir categóricas a dummies
  step_zv(all_predictors())                      # eliminar variables con varianza 0

# Modelo CART con parámetros a afinar
modelo_cart <- decision_tree(
  cost_complexity = tune(),  # parámetro cp
  tree_depth = tune()        # profundidad máxima del árbol
) %>%
  set_engine("rpart") %>%
  set_mode("regression")

# Workflow que junta receta y modelo
...
coords <- st_coordinates(train_espacial)

# Definir número de folds (puedes usar 2 para hacerlo más rápido)
set.seed(123)
k_folds <- 5
clusters <- kmeans(coords, centers = k_folds, nstart = 25)

# Agregar los clusters como folds al dataset
train_espacial$fold_id <- clusters$cluster

# Verifica cuántos hay por fold
table(train_espacial$fold_id)
...
for (i in 1:k_folds) {
  ...
  # Dividir en entrenamiento y prueba según fold
  train_fold <- train_espacial %>% filter(fold_id != i)
  test_fold  <- train_espacial %>% filter(fold_id == i)
  
  # Entrenar modelo (aquí ejemplo con CART)
  modelo <- rpart(price ~ ., data = train_fold %>% st_drop_geometry(), method = "class")
  
  # Predecir sobre test fold
  predicciones <- predict(modelo, newdata = test_fold %>% st_drop_geometry(), type = "class")
  
  # Guardar resultados
  resultados[[i]] <- data.frame(
    id = test_fold$id,  # Cambia por ID si tienes una variable que lo identifica
    real = test_fold$price,
...
# Evaluación general
table(resultados_df$real, resultados_df$predicho)
...
```{r}
# 2.1 Generar predicción
# Preparar los datos de prueba (por ejemplo, test_espacial)
# IMPORTANTE: Eliminar geometry y tener las mismas variables que en entrenamiento
test_cart <- test_espacial %>%
  select(-geometry)  # Quitar geometry

# Hacer predicción de price
predicciones <- predict(modelo_final_cart, new_data = test_cart)

# Unir predicciones con los IDs para ver resultados
resultados_pred <- test_cart %>%
  select(property_id) %>%
...
# Crear folds espaciales (5 folds)
folds_spatial <- spatialEco::spatialBlock(train_factors, 
                                          x = "lot", y = "lan", 
                                          theRange = 0.01, # distancia del bloque, ajusta según escala
                                          k = 5, 
                                          selection = "random")

# Esto agrega a train_factors una columna fold con número de fold
train_factors$fold <- folds_spatial$foldID

# Entrenar y evaluar por folds:
library(dplyr)
library(purrr)

results <- map_df(1:5, function(k){
  train_fold <- filter(train_factors, fold != k)
  val_fold <- filter(train_factors, fold == k)
  
  mod <- rpart(formula_modelo, data = train_fold, method = "anova")
  pred_val <- predict(mod, newdata = val_fold)
  
  data.frame(fold = k,
             RMSE = sqrt(mean((val_fold$price - pred_val)^2)))
})

print(results)
mean(results$RMSE)  # RMSE promedio
...
# 9. Visualizar y seleccionar mejores parámetros
autoplot(resultados_1_RF)
show_best(resultados_1_RF, metric = "mae", n = 5)
best_params_RF <- select_best(resultados_1_RF, metric = "mae")

# 10. Finalizar y ajustar modelo final
...
library(vip)
vip(extract_fit_parsnip(final_model_RF), num_features = 20)
...
# 12. Predecir sobre conjunto de test (supongamos que tienes test_espacial)
predicciones_rf <- predict(final_model_rf, new_data = test_espacial)

# 13. Agregar predicciones al test
resultados_pred_rf <- test_espacial %>%
  select(property_id) %>%
  bind_cols(predicciones_rf) %>%
  rename(predicted_price = .pred)

head(resultados_pred_rf)
```
...
### 2.4.1 Entrenamiento CV Espacial
...
write.csv(pred_boosting1,
          "C:/Users/njaco/OneDrive/Documentos/Mestría en Economía Aplicada/Semestre 3/Big Data y Machine Learning/Repositorios/T3_BDML/03_Stores/10-04_G4_BOOSTING1.csv", row.names = FALSE, 
          quote = FALSE)

write.csv(pred_boosting2,
          "C:/Users/njaco/OneDrive/Documentos/Mestría en Economía Aplicada/Semestre 3/Big Data y Machine Learning/Repositorios/T3_BDML/03_Stores/10-04_G4_BOOSTING2.csv", row.names = FALSE, 
          quote = FALSE)
...
Ajuste de base de datos train y test
...
train_factors_lm <- train_localidades %>%
  ...
  mutate(across(all_of(cols_factor_lm), as.factor)) %>%
  na.omit() %>%
  as.data.frame()

# Quitar NAs del dataframe preparado
train_factors_lm <- na.omit(train_factors_lm) ## Esta es la base que se usará para el entrenamiento
...
modelo_lm1 <- lm(formula_lm, data = train_factors_lm)
summary(modelo_lm1)

modelo_lm2 <- lm(price ~ year + rooms + bathrooms  + surface_covered + property_type  + distancia_mall + distancia_bus + distancia_avenida + binaria_parking + binaria_gym  + binaria_elevator, data = train_factors_lm)
summary(modelo_lm2)


library(vip)
vip(modelo_lm1, num_features = 20)
vip(modelo_lm2, num_features = 20)

modelo_lm3 <- lm(price ~  bathrooms  + surface_covered  + distancia_avenida + binaria_elevator +rooms, data = train_factors_lm)
summary(modelo_lm3)
...
train_factors_lm_log <- train_factors_lm %>%
...
```{r}
# Predicción
pred_lm1 <- predict(modelo_lm1, newdata = test_lm_input)

resultado_lm1 <- test_lm_input %>%
  select(property_id) %>%
  bind_cols(data.frame(price = pred_lm1))

# Ver la distribución de las predicciones
ggplot(resultado_lm1, aes(x = price/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
...
```{r}
# Predicción
pred_lm2 <- predict(modelo_lm2, newdata = test_lm_input)

resultado_lm2 <- test_lm_input %>%
  select(property_id) %>%
  bind_cols(data.frame(price = pred_lm2))

# Ver la distribución de las predicciones
ggplot(resultado_lm2, aes(x = price/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
...
```{r}
# Predicción
pred_lm3 <- predict(modelo_lm3, newdata = test_lm_input)

resultado_lm3 <- test_lm_input %>%
  select(property_id) %>%
...
# Ver la distribución de las predicciones
ggplot(resultado_log1, aes(x = price/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
...
# Ver la distribución de las predicciones
ggplot(resultado_log2, aes(x = price/1000000)) +  # Convertir a millones
  geom_histogram(bins = 30, fill = "skyblue", color = "darkblue") +
  labs(title = "Distribución de las predicciones de precio",
       x = "Precio predicho (Millones de Pesos)",
       y = "Frecuencia") +
